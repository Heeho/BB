...
open class Application(
    ...
    protected val packet = Packet()
    protected val hello = Hello()
    protected val here = Here()
    protected val event = Event()
    protected val ack = Ack()
    protected val bye = Bye()
    ...
    protected fun decodepacket(): Boolean {
        decoder.reset()
        packet.deserialize(decoder)
        decoder.reset()
        println("${this::class.java} received packet: ${Packet.Type.values().first { it.id == packet.type }}")
        return packet.protocolversion != protocolversion
    }
    ...
    protected fun newclientid() = UUID.randomUUID().hashCode()
    ...
    open fun getappstatus() {
        ...
        println("--PACKETS")
        println("connreq: $hello")
        println("connreq: $here")
        println("clientcom: $event")
        println("clientcomack: $ack")
        println("clientcomack: $bye")
    }
}
